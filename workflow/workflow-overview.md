...
The model is locked once a conversation starts (use "New chat" to switch). The Planner chat selector persists through dispatch settings as `planner.model`; missing or invalid Planner values fall back to Claude, while Worker and Review dispatch settings keep their Codex defaults independently. Only one turn runs at a time — sending while the Planner is still responding is rejected. Codex and Qwen run as one-shot turns today (no cross-turn memory); conversational continuity is Claude-specific via session resume.
